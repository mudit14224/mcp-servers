...
function getApiKey(): string {
    ...
    if (!apiKey) {
        console.error("Cannot find GOOGLE_MAPS_API_KEY")
        process.exit(1);
    }
    return apiKey
}

// Get the API key using the function
const GOOGLE_MAPS_API_KEY = getApiKey(); 

// Define the tools
const GEOCODE_TOOL: Tool = {
    name: "maps_geocode",
    description: "Convert an address into geographic location", 
    inputSchema: {
        type: "object",
        properties: {
            address: {
...
async function handleGeocode(address: string): Promise<ToolResponseType> {
...
async function handleReverseGeocode(latitude: number, longitude: number): Promise<ToolResponseType> {
  const url = new URL("https://maps.googleapis.com/maps/api/geocode/json");
...
async function handleElevation(locations: Array<{ latitude: number; longitude: number }>): Promise<ToolResponseType> {
